class RecintosZoo {
    ...
    validarBioma(recinto, animal) {
        ...
        const isSingleBioma = animalBioma.split(" ").length === 1;
        if(isSingleBioma) {
            return recinto.bioma === animalBioma;
        }

        const biomaResintoWord = new Set(recinto.bioma.split(/\s+/));
        const biomaAnimalWord = animalBioma.split(/\s+/);

        return biomaAnimalWord.some(word => biomaResintoWord.has(word));
